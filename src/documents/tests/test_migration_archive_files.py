...
def archive_path_old(self):
    ...
    return (Path(settings.ARCHIVE_DIR) / fname).as_posix()


def archive_path_new(doc):
    if doc.archive_filename is not None:
        return (Path(settings.ARCHIVE_DIR) / str(doc.archive_filename)).as_posix()
    else:
        return None
...
def thumbnail_path(doc):
    ...
    return (Path(settings.THUMBNAIL_DIR) / file_name).as_posix()
...
simple_jpg = (Path(__file__).parent / "samples" / "simple.jpg").as_posix()
simple_pdf = (Path(__file__).parent / "samples" / "simple.pdf").as_posix()
simple_pdf2 = (
...
simple_txt = (Path(__file__).parent / "samples" / "simple.txt").as_posix()
simple_png = (Path(__file__).parent / "samples" / "simple-noalpha.png").as_posix()
simple_png2 = (Path(__file__).parent / "examples" / "no-text.png").as_posix()
...
class TestMigrateArchiveFiles(DirectoriesMixin, FileSystemAssertsMixin, TestMigrations):
    ...
    def testArchiveFilesMigrated(self):
        ...
        for doc in Document.objects.all():
            ...
            if doc.archive_checksum:
                self.assertIsFile(archive_path_new(doc))
                with Path(archive_path_new(doc)).open("rb") as f:
...
class TestMigrateArchiveFilesErrors(DirectoriesMixin, TestMigrations):
    ...
    def test_archive_missing(self):
        ...
        Path(archive_path_old(doc)).unlink()
...
class TestMigrateArchiveFilesBackwards(
    ...
    def testArchiveFilesReverted(self):
        ...
        for doc in Document.objects.all():
            ...
            if doc.archive_checksum:
                self.assertIsFile(archive_path_old(doc))
                with Path(archive_path_old(doc)).open("rb") as f:
                    archive_checksum = hashlib.md5(f.read()).hexdigest()
                self.assertEqual(archive_checksum, doc.archive_checksum)
