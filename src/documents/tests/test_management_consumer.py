...
class TestConsumer(DirectoriesMixin, ConsumerThreadMixin, TransactionTestCase):
    ...
    def test_consume_file_invalid_ext(self):
        ...
        f = (Path(self.dirs.consumption_dir) / "my_file.wow").as_posix()
        shutil.copy(self.sample_file, f)
    ...
    def test_mac_write(self):
        ...
        shutil.copy(
            self.sample_file,
            (Path(self.dirs.consumption_dir) / ".DS_STORE").as_posix(),
        )
        shutil.copy(
            self.sample_file,
            (Path(self.dirs.consumption_dir) / "my_file.pdf").as_posix(),
        )
        shutil.copy(
            self.sample_file,
            (Path(self.dirs.consumption_dir) / "._my_file.pdf").as_posix(),
        )
        shutil.copy(
            self.sample_file,
            (Path(self.dirs.consumption_dir) / "my_second_file.pdf").as_posix(),
        )
        shutil.copy(
            self.sample_file,
            (Path(self.dirs.consumption_dir) / "._my_second_file.pdf").as_posix(),
        )
    ...
    def test_consume_file_busy(self, open_mock):
        ...
        f = (Path(self.dirs.consumption_dir) / "my_file.pdf").as_posix()
        shutil.copy(self.sample_file, f)
...
class TestConsumerTags(DirectoriesMixin, ConsumerThreadMixin, TransactionTestCase):
    ...
    def test_consume_file_with_path_tags(self):
        ...
        path = (Path(self.dirs.consumption_dir) / "/".join(tag_names)).as_posix()
        Path(path).mkdir(parents=True, exist_ok=True)
        f = Path(path) / "my_file.pdf"
