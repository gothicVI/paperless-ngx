...
logger: logging.Logger = logging.getLogger("paperless.index")
...
def update_document(writer: AsyncWriter, doc: Document) -> None:
    tags: str = ",".join([t.name for t in doc.tags.all()])
    tags_ids: str = ",".join([str(t.id) for t in doc.tags.all()])
    notes: str = ",".join([str(c.note) for c in Note.objects.filter(document=doc)])
    custom_fields: str = ",".join(
        [str(c) for c in CustomFieldInstance.objects.filter(document=doc)],
    )
    custom_fields_ids: str = ",".join(
...
def get_permissions_criterias(user: User | None = None) -> list:
    user_criterias: list = [query.Term("has_owner", False)]
    if user is not None:
